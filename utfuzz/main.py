...
def main():
    ...
    if not args.skip_dialog:
        ...
        if output_dir is None:
            output_dir = project_dir / "utfuzz_tests"
    ...
    try:
        if requirements_file is None:
            ...
            python_manager.project_requirements_install(pathlib.Path(requirements_file))
    ...
    if len(analyze_targets) == 0:
        ...
        analyze_targets = [
            pathlib.Path(f).resolve().absolute() for f in analyze_targets
        ]
    ...
    save_config(
        ...
        java,
        sys_paths,
        [str(f) for f in analyze_targets],
